...
void CQOpenGL::GLScreenToCoord3D(QPoint p, Vec3D<>& RetVec, bool SetZDepthHere)
{
	...
	else if (lastZDepth > 1.0) lastZDepth = 1.0;
...
void CQOpenGL::GLDrawAxes() //draws axes
{
	...
	CGL_Utils::DrawSphere(Vec3D<>(0,0, 0), AxisScale*0.04);
	CGL_Utils::DrawArrowD(Vec3D<>(0,0,AxisScale*0.0305), Vec3D<>(0, 0, AxisScale));
	CGL_Utils::DrawArrowD(Vec3D<>(0,AxisScale*0.0305,0), Vec3D<>(0, AxisScale, 0));
	CGL_Utils::DrawArrowD(Vec3D<>(AxisScale*0.0305, 0,0), Vec3D<>(AxisScale, 0, 0));
