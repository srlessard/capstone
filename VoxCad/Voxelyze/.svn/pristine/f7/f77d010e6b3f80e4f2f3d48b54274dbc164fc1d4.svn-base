...
#define VEC3D_HYSTERESIS_FACTOR 1.05 /*how much a valuemust go past 1.0 to switch states*/
...
//	#define vfloat float
	static const vfloat MAX_ERROR_PERCENT = 1e-5;
	static const vfloat DISCARD_ANGLE_RAD = 1e-7; //Anything less than this angle can be considered 0
	static const vfloat SMALL_ANGLE_RAD = 5.48e-3; //Angles less than this get small angle approximations. To get: Root solve atan(t)/t-1+MAX_ERROR_PERCENT. From: MAX_ERROR_PERCENT = (t-atan(t))/t 
	static const vfloat W_THRESH_ACOS2SQRT = 0.99988; //Threshhold of w above which we can approximate acos(w) with sqrt(2-2w). To get: Root solve 1-sqrt(2-2wt)/acos(wt) - MAX_ERROR_PERCENT. From MAX_ERROR_PERCENT = (acos(wt)-sqrt(2-2wt))/acos(wt)
	//Set compiler to /fp:fast
