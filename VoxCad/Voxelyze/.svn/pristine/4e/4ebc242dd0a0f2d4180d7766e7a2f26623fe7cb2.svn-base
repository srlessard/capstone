...
void CVX_Environment::UpdateCurTemp(vfloat time)
{
	if (VaryTempEnabled){
		...
		for (int i = 0; i<(int)pObj->GetNumMaterials(); i++){ //now update the individual temperatures of each material (they can each have a different temperature)
			pObj->GetBaseMat(i)->SetCurMatTemp(TempBase + TempAmplitude*sin((2*3.1415926f/TempPeriod) * time + pObj->GetBaseMat(i)->GetMatTempPhase()));	//and update each one
		}
	}
	else {
		CurTemp = TempBase + TempAmplitude;
		for (int i = 0; i<pObj->GetNumMaterials(); i++){ //for each material...
			pObj->GetBaseMat(i)->SetCurMatTemp(CurTemp); //...update each one
		}
	}
