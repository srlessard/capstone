...
struct VoxState {
	VoxState(void) {Pos = Vec3D<double>(0,0,0); LinMom = Vec3D<double>(0,0,0); Angle = CQuat<double>(1.0, 0, 0, 0); AngMom = Vec3D<double>(0,0,0); Scale = Vec3D<>(0,0,0); ScaleMom = Vec3D<>(0,0,0); Vel = Vec3D<>(0,0,0); KineticEnergy = 0; /*PotentialEnergy = 0;*/ AngVel = Vec3D<>(0,0,0); ScaleVel = Vec3D<>(0,0,0);};
	VoxState(const VoxState& SIn) {*this = SIn;} //copy constructor
	inline VoxState& operator=(const VoxState& SIn) {Pos = SIn.Pos; LinMom = SIn.LinMom; Angle = SIn.Angle; AngMom = SIn.AngMom; Scale = SIn.Scale; ScaleMom = SIn.ScaleMom; Vel = SIn.Vel; KineticEnergy=SIn.KineticEnergy; /*PotentialEnergy=SIn.PotentialEnergy;*/ AngVel = SIn.AngVel; ScaleVel = SIn.ScaleVel; return *this; }; //overload equals
...
{
public:
	CVXS_Voxel(CVX_Sim* pSimIn, int SIndexIn, int XIndexIn, int MatIndexIn, const Vec3D<>& OriginalPosIn, const Vec3D<>& OriginalScaleIn);
public:
	~CVXS_Voxel(void);
